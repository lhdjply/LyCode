...
struct Workspace {
    ...
    bool isRemote() const { return !remoteSessionId.isEmpty(); }
...
struct Session {
    ...
    bool isSubagent() const { return kind == SessionKind::SubagentChild; }
    bool hasParent() const { return !parentSessionId.isEmpty(); }
    /// 是否需要用户立即处理。
