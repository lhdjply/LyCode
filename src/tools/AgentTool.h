// ZCode Qt — 子 Agent 工具（`Agent`，别名 `Task`）
//
...
//
// 与 npm 的对应关系见 tools/SubagentHost.h 与 AgentRuntime::launchSubagent()。
#pragma once
...
public:
    ...
    /// 把子代理结果渲染成回传给模型的内容。
    ///
    /// 格式与 npm 的 formatAgentOutputForModel 对齐：先正文，再一行用量摘要。
    /// npm 还会附一句"用 SendMessage 继续这个 agent"，但本实现没有 SendMessage
