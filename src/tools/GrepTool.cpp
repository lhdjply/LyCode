...
QJsonObject GrepTool::inputSchema() const
{
  ...
  properties.insert(QStringLiteral("glob"),
                    stringProp(QStringLiteral("只搜索匹配该 glob 的文件，如 `*.cpp`")));
  ...
  properties.insert(QStringLiteral("type"),
                    stringProp(QStringLiteral("文件类型过滤（仅 ripgrep 后端支持），如 `cpp`")));
